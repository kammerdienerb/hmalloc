...
int objmap_entry_write_record_coop_buff(const char *entry_path, void *buff, int n_coop_buff_bytes);
...
int objmap_entry_read_record_coop_buff(const char *entry_path, void *buff, int n_coop_buff_bytes) {
    ...
    n_read = 0;
    while ((n_read = read(fd, buff + n_read, sizeof(struct proc_object_map_record_t) - n_read)) > 0) {}
...
int objmap_entry_write_record_coop_buff(const char *entry_path, void *buff, int n_coop_buff_bytes) {
    ...
    if ((fd = open(entry_path, O_WRONLY)) < 0) {
        status = errno;
        errno  = 0;
        goto out;
    }

    n_written = write(fd, buff, n_coop_buff_bytes);
